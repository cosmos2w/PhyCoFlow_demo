...
Place each existing field/error colourbar with enough horizontal room for its mantissa, exponent and variable label. Pressure's large offset requires especially careful separation. Do not infer Pa or nondimensional pressure from the plotted multiplier; A07/source metadata must provide units.
...
Use one consistent sequential error palette and preserve the original normalization for each table. If the source already uses shared normalization, keep it. If it does not, flag the difference rather than silently impose one. **Never normalize columns independently** to make pressure errors visually comparable with species/velocity errors. Keep every number visible with adaptive black/white text contrast and a legible font floor.

Lighten the most saturated table backgrounds enough through the colormap choice to make the numeric values primary, but do not alter the metric-to-colour normalization or clip large entries. Use light cell separation rather than thick grid borders. Slightly emphasize the DMF-Gen row label by weight; do not conceal lower errors belonging to other methods or add best-in-column stars.
...
Place the shared method legend above the bar group with sufficient separation from the variable titles. The spectra below contain the selected methods actually shown, not all eight. Retain that membership and the original GT label/reference trace. Use thin distinguishable line styles without inserting markers on every frequency shell. Preserve all wiggles and the high-frequency tail: do not smooth, average or re-bin the spectra.

Align each spectrum with its corresponding LSD axis above. Keep log axes, tick values, spectral units and the existing wavenumber caption. The 256 T sensors and coverage annotation are source facts, not an invitation to recompute or round the percentage differently.
...
Keep the three JSD groups aligned with the corresponding PDFs. Retain all eight method violins, their row order, density paths and mean annotations. Use the same method colours as c and the other figures. Move the mean annotations into aligned gutters where they do not overlap tails. Preserve every value, including the better SiT result for CH4–U1.

Use a neutral base-2 JSD axis label with clearly separated ticks. Do not turn JSD into a percentage or force identical x limits if the existing comparisons use different ranges. The PDF maps are pooled over 25 states while the JSD distributions use 1,000 states according to the caption; no pooling or sampling change is allowed.
