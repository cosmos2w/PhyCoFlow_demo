#!/usr/bin/env python
"""Export V3-3 standalone panels, validated SI figures, and LaTeX tables."""
...
from common.publication_panels_unified_v3_3 import PANEL_OUTPUT_NAMES, draw_panel, panel_label
...
assembler = _load("unified_v3_3_assembler", HERE / "103_assemble_mixed_resolution_unified_v3_3.py")
exporter32 = _load("unified_v3_2_exporter", HERE / "101_export_unified_v3_2_panels.py")
base = exporter32.base


def _collect_cache_paths(payload):
    return exporter32._collect_cache_paths(payload)
...
def main():
    ...
    if not rid:
        raise ValueError("--run-id is required for additive V3-3 exports")
    ...
    root = FIGURES_DIR / "UnifiedV3_3" / rid
    if root.exists():
        raise FileExistsError(f"Refusing to overwrite existing V3-3 export directory: {root}")
    root.mkdir(parents=True)
    panel_outputs, panel_meta = export_standalone_panels(ctx, cfg, layout, root, rid)
    si_layout = dict(layout); si_layout["si_hybrid"] = dict(layout["si_v3_3"])
    si_outputs, si_meta = exporter32.export_si(ctx, cfg, si_layout, root, rid)
