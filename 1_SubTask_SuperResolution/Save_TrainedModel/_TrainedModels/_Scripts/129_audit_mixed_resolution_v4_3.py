#!/usr/bin/env python
"""Independent audit for the frozen-data mixed-resolution V4_3 bundle."""
...
V4_2_RELEASE = DEFAULT_RELEASE_ROOT / "MixedResolution_unified_v4_2_20260914_2200"
...
base = _load("mixed_resolution_v4_3_audit_helpers", HERE / "120_audit_mixed_resolution_v4.py")
...
def _source_lock_check(lock: dict, release: Path) -> tuple[bool, dict]:
    keys = ("v4_3_renderer", "v4_3_exporter", "v4_3_audit",
            "v4_3_panel_renderer", "v4_3_layout", "v4_3_wrapper")
    detail, passed = {}, True
    for key in keys:
        item = lock.get(key, {})
...
def audit(release_dir: Path, output: Path | None = None) -> dict:
    ...
    manifest = _read(release / "source_manifest_v4_3.json")
    ...
    baseline = _read(V4_2_RELEASE / "source_manifest_v4_2.json")
    pdfs = sorted(release.glob("MixedResolution_unified_v4_3_*.pdf"))
    svgs = sorted(release.glob("MixedResolution_unified_v4_3_*.svg"))
    pngs = sorted(release.glob("MixedResolution_unified_v4_3_*.png"))
    ...
    add("v4_3_release_identity_and_bookkeeping",
        required_ok and manifest.get("revision") == "V4_3" and release.name.startswith("MixedResolution_unified_v4_3_"),
    ...
    comparison_pass = (comparison.get("status") == "PASS"
                       ...
                       and comparison.get("v4_3_raw_or_scientific_deltas") is False
                       and bool(comparison_checks) and all(comparison_checks.values()))
    add("underlying_saved_data_exact_to_v4_2", comparison_pass,
    ...
    add("v4_3_panel_and_artist_inventory", inventory_pass, inventory)
    ...
    add("panel_a_tick_sized_captions_and_contrasting_lmh_labels", a_pass,
        {key: a.get(key) for key in ("resolution_caption_count", "resolution_caption_fontsize_pt",
                                     ...
                                     "interior_scale_identity_labels")})
    ...
    palette_pass = (lqa.get("grayscale_check", {}).get("passed") is True
                    ...
                    and lqa.get("normalization_unchanged") is False
                    and lqa.get("normalization_change", {}).get("authorized") is True
                    and lqa.get("normalization_change", {}).get("vmin") == 0.0)
    ...
    payload = {"workflow_label": "mixed_resolution_unified_v4_3_audit",
               "schema_version": "4.3", "revision": "V4_3",
               "status": "PASS" if passed else "FAIL", "passed": passed,
               "release_dir": str(release), "checks": checks}
    destination = Path(output).resolve() if output else release / "qa_v4_3.json"
