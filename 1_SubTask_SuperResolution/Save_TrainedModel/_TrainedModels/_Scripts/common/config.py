...
def run_id(value: str | None = None) -> str:
    ...
    return value or datetime.now().strftime("%Y%m%d_%H%M")
...
def ensure_output_dirs() -> None:
    for rel in (
        "ModelInventory", "DatasetStats", "CanonicalTestIndex", "SensorPlans",
        "ReconstructionCache", "ResolutionProtocol", "QuestionA_DataBenefit",
        "CoarseDetail", "QuestionB_ZeroH", "FrequencyError", "GradientError",
        "SensorSweep", "MultiscaleWavelet",
    ):
        (RESULTS_DIR / rel).mkdir(parents=True, exist_ok=True)
    for rel in (
        "_Contours", "ResolutionProtocol", "QuestionA_DataBenefit", "CoarseDetail",
        "QuestionB_ZeroH", "FrequencyError", "GradientError", "SensorSweep", "Assembled",
        "MultiscaleWavelet",
    ):
...
def method_items(cfg: dict[str, Any], selected: Iterable[str] | None = None):
    ...
    for method in cfg["models"]:
        keys = {method["key"].lower(), method["label"].lower(), method["directory"].lower(), *(a.lower() for a in method.get("aliases", []))}
...
def recipe_items(cfg: dict[str, Any], selected: Iterable[str] | None = None):
    wanted = None if selected is None else {str(x).lower() for x in selected}
    for key, spec in cfg["recipes"].items():
        aliases = spec.get("directory_aliases", [])
        keys = {key.lower(), str(spec["label"]).lower(), *(str(a).lower() for a in aliases)}
        if wanted is None or "all" in wanted or keys & wanted:
            yield key, spec
