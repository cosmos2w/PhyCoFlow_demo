...
def matching_or_latest(directory: Path, prefix: str, run_id: str | None, suffix: str) -> Path:
    """Prefer the exact run artifact; use latest only for interactive convenience."""
    if run_id:
        exact = directory / artifact_name(prefix, run_id, suffix)
        if exact.exists():
            return exact
    return latest(directory, prefix, suffix)
