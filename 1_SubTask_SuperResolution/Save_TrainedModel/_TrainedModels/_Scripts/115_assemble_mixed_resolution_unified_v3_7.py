#!/usr/bin/env python
"""Assemble the additive five-panel mixed-resolution V3-7 figure."""
...
from common.publication_panels_unified_v3_7 import draw_panel, panel_label
from matplotlib.colors import to_hex

HERE = Path(__file__).resolve().parent
BASE_ASSEMBLER = HERE / "112_assemble_mixed_resolution_unified_v3_6.py"
_spec = importlib.util.spec_from_file_location("unified_v3_6_base_assembler", BASE_ASSEMBLER)
if _spec is None or _spec.loader is None:
    raise RuntimeError(f"Cannot load V3-6 base assembler: {BASE_ASSEMBLER}")
base36 = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(base36)
base = base36.base


def _geometry(layout):
    cfg = layout["v3_7_geometry"]
    ...
    if abs(c_width + spacer + d_width - width) > 1e-9:
        raise ValueError("V3-7 c/spacer/d widths do not close to the canvas width")
    if abs(heights["a"] + vgap + heights["b"] + vgap + heights["cd"] + vgap + heights["e"] - height) > 1e-9:
        raise ValueError("V3-7 row heights do not close to the canvas height")
    e_bottom = 0.0
...
def create_canvas(layout):
    ...
    containers, axes = {}, {}
    content = layout["v3_7_geometry"]["content_bounds"]
    ...
    shared_cfg = layout["v3_7_geometry"]["shared_cd_grid"]
    cd_bottom = rects["c"][1]
    ...
    if row_height <= 0:
        raise ValueError("V3-7 shared c+d visual rows have non-positive height")
...
def create_standalone_canvas(layout, label):
    ...
    ax = container.inset_axes(
        list(map(float, layout["v3_7_geometry"]["content_bounds"][label])),
...
def _panel_cd_alignment_qa(fig, shared_axes, export_dpi):
    ...
    exact_bottom_pixels = all(row["bottom_export_pixel_delta"] == 0 for row in rows)
    c_boundaries = [[row["c_bottom_mm"], row["c_top_mm"]] for row in rows]
    d_boundaries = [[row["d_bottom_mm"], row["d_top_mm"]] for row in rows]
    return {
        "passed": exact_shared_boundaries and exact_bottom_pixels,
        ...
        "bottom_row_export_pixel_alignment": {
            "passed": rows[-1]["bottom_export_pixel_delta"] == 0,
            "c_bottom_pixel": rows[-1]["c_bottom_export_pixel"],
            "d_bottom_pixel": rows[-1]["d_bottom_export_pixel"],
            "delta_pixels": rows[-1]["bottom_export_pixel_delta"],
        },
        "v3_7_row_boundaries_mm": {"c": c_boundaries, "d": d_boundaries},
    }
...
def _v3_6_anchor():
    """Verify the exact local V3-6 stack before any V3-7 build is attempted."""
    manifest_path = FIGURES_DIR / "Assembled" / "FigureSourceManifest_unified_v3_6_20260914_1129.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    main_base = FIGURES_DIR / "Assembled" / "MixedResolution_unified_v3_6_20260914_1129"
    wrapper = HERE.parents[2] / "figures" / "scripts" / "render_mixed_resolution_v3_6.py"
    records = {
        ...
        "assembler": base.record(HERE / "112_assemble_mixed_resolution_unified_v3_6.py"),
        "exporter": base.record(HERE / "113_export_unified_v3_6_panels.py"),
        "audit": base.record(HERE / "114_audit_unified_v3_6.py"),
        "wrapper": base.record(wrapper),
        "panel_renderer": base.record(HERE / "common" / "publication_panels_unified_v3_6.py"),
        "layout": base.record(HERE / "publication_layout_unified_v3_6.yaml"),
    }
    ...
    if not matches:
        raise RuntimeError("Local V3-6 renderer stack or baseline no longer matches the 20260914_1129 manifest")
    return records
...
def build_manifest(ctx, cfg, layout, layout_path, outputs, panel_meta, rects, width, height,
    ...
    v3_6_manifest = FIGURES_DIR / "Assembled" / "FigureSourceManifest_unified_v3_6_20260914_1129.json"
    v3_6_base = FIGURES_DIR / "Assembled" / "MixedResolution_unified_v3_6_20260914_1129"
    v3_6_release = HERE.parents[2] / "figures" / "generated" / "MixedResolution_unified_v3_6_20260914_1129"
    ...
    return {
        ...
        "figure_contract": {
            "core_conclusion": (
                "V3-7 preserves the exact V3-6 mixed-resolution scientific conclusion: L/M/H encode distinct "
        ...
        "selection_contract": {
            ...
            "panel_b": {"sensor_count": 512, "recipes": layout["panel_b_v3_7"]["recipes"],
                        "sweep_recipes": layout["panel_b_v3_7"]["sweep_recipes"]},
            "panel_c": {"recipe": layout["panel_c_v3_7"]["recipe"], "sensor_count": 512,
                        "models": layout["panel_c_v3_7"]["models"],
        ...
        "panel_renderer": base.record(HERE / "common" / "publication_panels_unified_v3_7.py"),
        "v3_6_starting_renderer_anchor": _v3_6_anchor(),
        "v3_6_baseline": {
            "manifest": base.record(v3_6_manifest),
            "main_pdf": base.record(v3_6_base.with_suffix(".pdf")),
            "main_svg": base.record(v3_6_base.with_suffix(".svg")),
            "main_png": base.record(v3_6_base.with_suffix(".png")),
        },
        "v3_6_hash_anchor": [
            base.record(v3_6_release / "MixedResolution_unified_v3_6_20260914_1129.pdf"),
            base.record(v3_6_release / "MixedResolution_unified_v3_6_20260914_1129.svg"),
            base.record(v3_6_release / "MixedResolution_unified_v3_6_20260914_1129.png"),
            base.record(v3_6_release / "source_manifest_v3_6.json"),
            base.record(v3_6_release / "qa_v3_6.json"),
        ],
        "v3_5_baseline": {
        ...
        "panel_a_style_reference": {
            ...
            "implementation_basis": (
                "The V3-6 nested-inset treatment is retained as the starting visual language; V3-7 changes "
...
def main():
    ...
    if not model_artist_qa["passed"]:
        raise ValueError(f"V3-7 model artist contract failed: {model_artist_qa}")
    geometry_qa = _geometry_qa(fig, containers, rects)
    if not geometry_qa["passed"]:
        raise ValueError(f"V3-7 geometry failed: {geometry_qa}")
    alignment_qa = _panel_cd_alignment_qa(
        fig, shared_axes, cfg["figure_style"]["paper_dpi"]
    )
    if not alignment_qa["passed"]:
        raise ValueError(f"V3-7 shared c+d row alignment failed: {alignment_qa}")
    ...
    out = FIGURES_DIR / "Assembled" / f"MixedResolution_unified_v3_7_{rid}"
