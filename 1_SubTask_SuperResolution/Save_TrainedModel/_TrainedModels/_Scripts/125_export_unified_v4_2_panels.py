#!/usr/bin/env python
"""Export standalone V4_2 panels and preserve the validated V4_1 SI bundle."""
...
from common.publication_panels_unified_v4_2 import (
    PANEL_OUTPUT_NAMES,
    draw_panel,
    panel_label,
)
...
assembler = _load("unified_v4_2_assembler_for_export", HERE / "124_assemble_mixed_resolution_unified_v4_2.py")
exporter37 = _load("unified_v3_7_exporter_for_v4_2", HERE / "116_export_unified_v3_7_panels.py")
...
def export_standalone_panels(ctx, cfg, layout, root: Path, rid: str):
    ...
    for label in "abcde":
        fig, ax, _container = assembler._create_standalone_canvas_v4_2(layout, label)
...
def main():
    ...
    rid = args.run_id
    if not rid:
        raise ValueError("--run-id is required for additive V4_2 exports")
    ...
    root = FIGURES_DIR / "UnifiedV4_2" / rid
    if root.exists():
        raise FileExistsError(f"Refusing to overwrite existing V4_2 export directory: {root}")
    root.mkdir(parents=True)
    panel_outputs, panel_meta = export_standalone_panels(ctx, cfg, layout, root, rid)

    for name in ("si", "tables"):
        source = V4_1_RELEASE / name
        if not source.is_dir():
            raise FileNotFoundError(f"Validated V4_1 {name} directory is missing: {source}")
    ...
    manifest["si_outputs"] = {
        "preserved_v4_1": [base.record(path) for path in sorted((root / "si").rglob("*")) if path.is_file()]
    }
    manifest["si_metadata"] = {
        "source_revision": "V4_1", "restyled": False,
        "reason": "SI is outside the active V4_2 main-figure scope",
        "source_directory": str((V4_1_RELEASE / "si").resolve()),
    }
    manifest["table_outputs"] = {
        "preserved_v4_1": [base.record(path) for path in sorted((root / "tables").rglob("*")) if path.is_file()]
    }
    v4_manifest = json.loads((V4_1_RELEASE / "source_manifest_v4_1.json").read_text(encoding="utf-8"))
    for key in (
