#!/usr/bin/env python
"""Assemble the additive five-panel mixed-resolution V3-5 figure."""
...
from common.publication_panels_unified_v3_5 import draw_panel, panel_label


HERE = Path(__file__).resolve().parent
BASE_ASSEMBLER = HERE / "106_assemble_mixed_resolution_unified_v3_4.py"
_spec = importlib.util.spec_from_file_location("unified_v3_4_base_assembler", BASE_ASSEMBLER)
if _spec is None or _spec.loader is None:
    raise RuntimeError(f"Cannot load V3-4 base assembler: {BASE_ASSEMBLER}")
base34 = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(base34)
base = base34.base


def _geometry(layout):
    cfg = layout["v3_5_geometry"]
    ...
    if abs(c_width + spacer + d_width - width) > 1e-9:
        raise ValueError("V3-5 c/spacer/d widths do not close to the canvas width")
    if abs(heights["a"] + vgap + heights["b"] + vgap + heights["cd"] + vgap + heights["e"] - height) > 1e-9:
        raise ValueError("V3-5 row heights do not close to the canvas height")
    e_bottom = 0.0
...
def create_canvas(layout):
    ...
    containers, axes = {}, {}
    content = layout["v3_5_geometry"]["content_bounds"]
    ...
    shared_cfg = layout["v3_5_geometry"]["shared_cd_grid"]
    cd_bottom = rects["c"][1]
    ...
    if row_height <= 0:
        raise ValueError("V3-5 shared c+d visual rows have non-positive height")
...
def create_standalone_canvas(layout, label):
    ...
    ax = container.inset_axes(
        list(map(float, layout["v3_5_geometry"]["content_bounds"][label])),
...
def _panel_cd_alignment_qa(fig, shared_axes):
    ...
    return {
        ...
        "v3_5_row_boundaries_mm": {"c": c_boundaries, "d": d_boundaries},
    }
...
def _v3_4_anchor():
    """Verify the exact local V3-4 stack before any V3-5 build is attempted."""
    manifest_path = FIGURES_DIR / "Assembled" / "FigureSourceManifest_unified_v3_4_20260914_1040.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    main_base = FIGURES_DIR / "Assembled" / "MixedResolution_unified_v3_4_20260914_1040"
    wrapper = HERE.parents[2] / "figures" / "scripts" / "render_mixed_resolution_v3_4.py"
    records = {
        ...
        "assembler": base.record(HERE / "106_assemble_mixed_resolution_unified_v3_4.py"),
        "exporter": base.record(HERE / "107_export_unified_v3_4_panels.py"),
        "audit": base.record(HERE / "108_audit_unified_v3_4.py"),
        "wrapper": base.record(wrapper),
        "panel_renderer": base.record(HERE / "common" / "publication_panels_unified_v3_4.py"),
        "layout": base.record(HERE / "publication_layout_unified_v3_4.yaml"),
    }
    ...
    if not matches:
        raise RuntimeError("Local V3-4 renderer stack or baseline no longer matches the 20260914_1040 manifest")
    return records
...
def build_manifest(ctx, cfg, layout, layout_path, outputs, panel_meta, rects, width, height,
    ...
    return {
        ...
        "figure_contract": {
            "core_conclusion": (
                "V3-5 preserves the validated mixed-resolution scientific conclusion: L/M/H encode distinct "
            ...
            "hero_evidence": ["b", "c", "e"],
            "revision_scope": "geometric harmonization and matrix-layout revision only; validated scientific content and selection are unchanged",
        ...
        "selection_contract": {
            ...
            "panel_b": {"sensor_count": 512, "recipes": layout["panel_b_v3_5"]["recipes"],
                        "sweep_recipes": layout["panel_b_v3_5"]["sweep_recipes"]},
            "panel_c": {"recipe": layout["panel_c_v3_5"]["recipe"], "sensor_count": 512,
                        "models": layout["panel_c_v3_5"]["models"],
        ...
        "v3_4_starting_renderer_anchor": _v3_4_anchor(),
        "v3_4_baseline": {
        ...
        "panel_a_style_reference": {
            ...
            "implementation_basis": (
                "The V3-4 nested-inset treatment is retained as the starting visual language; V3-5 changes "
...
def main():
    ...
    parser.add_argument("--layout", type=Path, default=HERE / "publication_layout_unified_v3_5.yaml")
    ...
    if not model_artist_qa["passed"]:
        raise ValueError(f"V3-5 model artist contract failed: {model_artist_qa}")
    geometry_qa = _geometry_qa(fig, containers, rects)
    if not geometry_qa["passed"]:
        raise ValueError(f"V3-5 geometry failed: {geometry_qa}")
    alignment_qa = _panel_cd_alignment_qa(fig, shared_axes)
    if not alignment_qa["passed"]:
        raise ValueError(f"V3-5 shared c+d row alignment failed: {alignment_qa}")
    ...
    out = FIGURES_DIR / "Assembled" / f"MixedResolution_unified_v3_5_{rid}"
    ...
    manifest_path = FIGURES_DIR / "Assembled" / f"FigureSourceManifest_unified_v3_5_{rid}.json"
