#!/usr/bin/env python
"""Assemble the additive five-panel mixed-resolution V3-4 figure."""
...
from common.publication_panels_unified_v3_4 import draw_panel, panel_label


HERE = Path(__file__).resolve().parent
BASE_ASSEMBLER = HERE / "103_assemble_mixed_resolution_unified_v3_3.py"
_spec = importlib.util.spec_from_file_location("unified_v3_3_base_assembler", BASE_ASSEMBLER)
if _spec is None or _spec.loader is None:
    raise RuntimeError(f"Cannot load V3-3 base assembler: {BASE_ASSEMBLER}")
base33 = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(base33)
base = base33.base


def _geometry(layout):
    cfg = layout["v3_4_geometry"]
    ...
    if abs(left + hgap + right - width) > 1e-9:
        raise ValueError("V3-4 lower widths do not close to the canvas width")
    if abs(heights["a"] + vgap + heights["b"] + vgap + heights["c"] + vgap + heights["e"] - height) > 1e-9:
        raise ValueError("V3-4 row heights do not close to the canvas height")
    e_bottom = 0.0
...
def create_canvas(layout):
    ...
    containers, axes = {}, {}
    content = layout["v3_4_geometry"]["content_bounds"]
...
def create_standalone_canvas(layout, label):
    ...
    ax = container.inset_axes(
        list(map(float, layout["v3_4_geometry"]["content_bounds"][label])),
...
def _panel_cd_alignment_qa(layout, rects):
    """Compare c/d image-row center alignment against the exact V3-3 layout."""
    def centers(layout_here, rects_here, geometry_key, c_key, d_key):
        content = layout_here[geometry_key]["content_bounds"]

        def absolute_y(panel, relative_y):
            _left, bottom, _width, height = rects_here[panel]
            cb = content[panel]
            return bottom + height * (float(cb[1]) + float(cb[3]) * relative_y)

        c_cfg = layout_here[c_key]
        c_centers = [
            absolute_y("c", float(c_cfg[f"{row}_row_bottom"]) + float(c_cfg[f"{row}_row_height"]) / 2)
            for row in ("full", "zoom", "error")
        ]
        d_cfg = layout_here[d_key]
        _qx, qy, _qw, qh = map(float, d_cfg["qualitative_bounds"])
        gap = float(d_cfg["qualitative_row_gap"])
        row_height = (qh - 2 * gap) / 3
        d_centers = [
            absolute_y("d", qy + (2 - index) * (row_height + gap) + row_height / 2)
            for index in range(3)
        ]
        return c_centers, d_centers

    current_c, current_d = centers(
        layout, rects, "v3_4_geometry", "panel_c_v3_4", "panel_d_v3_4"
    )
    old_layout = base.load_layout(HERE / "publication_layout_unified_v3_3.yaml")
    _old_width, _old_height, old_rects = base33._geometry(old_layout)
    old_c, old_d = centers(
        old_layout, old_rects, "v3_3_geometry", "panel_c_v3_3", "panel_d_v3_3"
    )
    current_delta = [abs(left - right) for left, right in zip(current_c, current_d)]
    old_delta = [abs(left - right) for left, right in zip(old_c, old_d)]
    return {
        "passed": max(current_delta) < max(old_delta),
        "row_order": ["full/large", "zoom/intermediate", "local-error/fine"],
        "v3_4_centers_mm": {"c": current_c, "d": current_d},
        "v3_3_centers_mm": {"c": old_c, "d": old_d},
        "v3_4_absolute_deltas_mm": current_delta,
        "v3_3_absolute_deltas_mm": old_delta,
        "v3_4_max_delta_mm": max(current_delta),
        "v3_3_max_delta_mm": max(old_delta),
    }
...
def _v3_3_anchor():
    """Verify the exact local V3-3 stack before any V3-4 build is attempted."""
    manifest_path = FIGURES_DIR / "Assembled" / "FigureSourceManifest_unified_v3_3_20260913_2352.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    main_base = FIGURES_DIR / "Assembled" / "MixedResolution_unified_v3_3_20260913_2352"
    wrapper = HERE.parents[2] / "figures" / "scripts" / "render_mixed_resolution_v3_3.py"
    records = {
        ...
        "assembler": base.record(HERE / "103_assemble_mixed_resolution_unified_v3_3.py"),
        "exporter": base.record(HERE / "104_export_unified_v3_3_panels.py"),
        "audit": base.record(HERE / "105_audit_unified_v3_3.py"),
        "wrapper": base.record(wrapper),
        "panel_renderer": base.record(HERE / "common" / "publication_panels_unified_v3_3.py"),
        "layout": base.record(HERE / "publication_layout_unified_v3_3.yaml"),
    }
    ...
    if not matches:
        raise RuntimeError("Local V3-3 renderer stack or baseline no longer matches the 20260913_2352 manifest")
    return records
...
def build_manifest(ctx, cfg, layout, layout_path, outputs, panel_meta, rects, width, height, qa, args, rid):
    ...
    return {
        ...
        "figure_contract": {
            "core_conclusion": (
                "V3-4 preserves the validated mixed-resolution scientific conclusion: L/M/H encode distinct "
            ...
            "hero_evidence": ["b", "c"],
            "revision_scope": "publication polish only; validated scientific content and selection are unchanged",
            "image_integrity": (
        ...
        "selection_contract": {
            ...
            "panel_b": {"sensor_count": 512, "recipes": layout["panel_b_v3_4"]["recipes"],
                        "sweep_recipes": layout["panel_b_v3_4"]["sweep_recipes"]},
            "panel_c": {"recipe": layout["panel_c_v3_4"]["recipe"], "sensor_count": 512,
                        "models": layout["panel_c_v3_4"]["models"],
        ...
        "cross_panel_alignment_qa": _panel_cd_alignment_qa(layout, rects),
        "panel_cd_rows_aligned": _panel_cd_alignment_qa(layout, rects)["passed"],
        "layout": {
        ...
        "panel_renderer": base.record(HERE / "common" / "publication_panels_unified_v3_4.py"),
        "v3_3_starting_renderer_anchor": _v3_3_anchor(),
        "v3_3_baseline": {
        ...
        "panel_a_style_reference": {
            ...
            "implementation_basis": (
                "The requested attachment was unavailable in the mounted environment. "
                "The explicit V3-4 geometry contract and the retained old panel-a render "
                "were used for the nested bottom-right inset treatment."
            ),
        },
...
def main():
    ...
    parser.add_argument("--layout", type=Path, default=HERE / "publication_layout_unified_v3_4.yaml")
    ...
    if not model_artist_qa["passed"]:
        raise ValueError(f"V3-4 model artist contract failed: {model_artist_qa}")
    geometry_qa = _geometry_qa(fig, containers, rects)
    if not geometry_qa["passed"]:
        raise ValueError(f"V3-4 geometry failed: {geometry_qa}")
    ...
    out = FIGURES_DIR / "Assembled" / f"MixedResolution_unified_v3_4_{rid}"
    ...
    manifest_path = FIGURES_DIR / "Assembled" / f"FigureSourceManifest_unified_v3_4_{rid}.json"
