#!/usr/bin/env python
"""Export V3-4 standalone panels, validated SI figures, and LaTeX tables."""
...
from common.publication_panels_unified_v3_4 import PANEL_OUTPUT_NAMES, draw_panel, panel_label
...
assembler = _load("unified_v3_4_assembler", HERE / "106_assemble_mixed_resolution_unified_v3_4.py")
exporter33 = _load("unified_v3_3_exporter", HERE / "104_export_unified_v3_3_panels.py")
base = exporter33.base


def _collect_cache_paths(payload):
    return exporter33._collect_cache_paths(payload)
...
def main():
    ...
    if not rid:
        raise ValueError("--run-id is required for additive V3-4 exports")
    ...
    root = FIGURES_DIR / "UnifiedV3_4" / rid
    if root.exists():
        raise FileExistsError(f"Refusing to overwrite existing V3-4 export directory: {root}")
    root.mkdir(parents=True)
    panel_outputs, panel_meta = export_standalone_panels(ctx, cfg, layout, root, rid)
    si_layout = dict(layout); si_layout["si_hybrid"] = dict(layout["si_v3_4"])
    # V3-3 intentionally delegates the validated SI export implementation to
    # its V3-2 exporter; preserve that exact path instead of duplicating it.
    si_outputs, si_meta = exporter33.exporter32.export_si(ctx, cfg, si_layout, root, rid)
