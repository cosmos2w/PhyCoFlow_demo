#!/usr/bin/env python
"""Export standalone V4_5 panels and preserve the validated V4_4 SI bundle."""
...
from common.publication_panels_unified_v4_5 import (
    PANEL_OUTPUT_NAMES, apply_v4_5_typography, draw_panel, panel_label,
...
assembler = _load("unified_v4_5_assembler_for_export", HERE / "133_assemble_mixed_resolution_unified_v4_5.py")
exporter37 = _load("unified_v3_7_exporter_for_v4_5", HERE / "116_export_unified_v3_7_panels.py")
...
def export_standalone_panels(ctx, cfg, layout, root: Path, rid: str):
    ...
    for label in "abcde":
        fig, ax, _container = assembler._create_standalone_canvas_v4_5(layout, label)
        panel_label(ax, label)
        metadata[label] = draw_panel(label, ax, ctx)
        assembler.activate_v4_5_font_roles(cfg)
        apply_v4_5_typography(fig)
...
def main():
    ...
    rid = args.run_id
    if not rid:
        raise ValueError("--run-id is required for additive V4_5 exports")
    cfg = load_config(args.config)
    assembler.apply_v4_5_style_contract(cfg)
    ensure_output_dirs()
    layout = base.load_layout(Path(args.layout).resolve())
    ctx = base.make_context(args, cfg, layout, rid)
    root = FIGURES_DIR / "UnifiedV4_5" / rid
    if root.exists():
        raise FileExistsError(f"Refusing to overwrite existing V4_5 export directory: {root}")
    root.mkdir(parents=True)
    panel_outputs, panel_meta = export_standalone_panels(ctx, cfg, layout, root, rid)
    for name in ("si", "tables"):
        source = V4_4_RELEASE / name
        if not source.is_dir():
            raise FileNotFoundError(f"Validated V4_4 {name} directory is missing: {source}")
    ...
    manifest["si_outputs"] = {
        "preserved_v4_4": [base.record(path) for path in sorted((root / "si").rglob("*")) if path.is_file()]
    }
    manifest["si_metadata"] = {
        "source_revision": "V4_4", "restyled": False,
        "reason": "SI is outside the active V4_5 main-figure scope",
        "source_directory": str((V4_4_RELEASE / "si").resolve()),
    }
    manifest["table_outputs"] = {
        "preserved_v4_4": [base.record(path) for path in sorted((root / "tables").rglob("*")) if path.is_file()]
    }
    prior = json.loads((V4_4_RELEASE / "source_manifest_v4_4.json").read_text(encoding="utf-8"))
