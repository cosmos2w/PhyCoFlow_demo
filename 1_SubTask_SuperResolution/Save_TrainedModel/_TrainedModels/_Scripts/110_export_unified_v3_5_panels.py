#!/usr/bin/env python
"""Export V3-5 standalone panels, validated SI figures, and LaTeX tables."""
...
from common.publication_panels_unified_v3_5 import PANEL_OUTPUT_NAMES, draw_panel, panel_label
...
assembler = _load("unified_v3_5_assembler", HERE / "109_assemble_mixed_resolution_unified_v3_5.py")
exporter34 = _load("unified_v3_4_exporter", HERE / "107_export_unified_v3_4_panels.py")
base = exporter34.base


def _collect_cache_paths(payload):
    return exporter34._collect_cache_paths(payload)
...
def main():
    ...
    if not rid:
        raise ValueError("--run-id is required for additive V3-5 exports")
    ...
    root = FIGURES_DIR / "UnifiedV3_5" / rid
    if root.exists():
        raise FileExistsError(f"Refusing to overwrite existing V3-5 export directory: {root}")
    root.mkdir(parents=True)
    panel_outputs, panel_meta = export_standalone_panels(ctx, cfg, layout, root, rid)
    si_layout = dict(layout); si_layout["si_hybrid"] = dict(layout["si_v3_5"])
    # V3-4 delegates the validated SI export implementation to its earlier
    # exporter; preserve that exact source path for V3-5 as well.
    si_outputs, si_meta = exporter34.exporter33.exporter32.export_si(ctx, cfg, si_layout, root, rid)
