#!/usr/bin/env python
"""Export V3-6 standalone panels, validated SI figures, and LaTeX tables."""
...
from common.publication_panels_unified_v3_6 import (
    PANEL_OUTPUT_NAMES,
    draw_panel,
    panel_label,
)
...
assembler = _load("unified_v3_6_assembler", HERE / "112_assemble_mixed_resolution_unified_v3_6.py")
exporter35 = _load("unified_v3_5_exporter", HERE / "110_export_unified_v3_5_panels.py")
base = exporter35.base


def _collect_cache_paths(payload):
    return exporter35._collect_cache_paths(payload)
...
def main():
    ...
    if not rid:
        raise ValueError("--run-id is required for additive V3-6 exports")
    ...
    root = FIGURES_DIR / "UnifiedV3_6" / rid
    if root.exists():
        raise FileExistsError(f"Refusing to overwrite existing V3-6 export directory: {root}")
    root.mkdir(parents=True)
    panel_outputs, panel_meta = export_standalone_panels(ctx, cfg, layout, root, rid)
    si_layout = dict(layout); si_layout["si_hybrid"] = dict(layout["si_v3_6"])
    # V3-5 delegates the validated SI export implementation to its earlier
    # exporter; preserve that exact source path for V3-6 as well.
    si_outputs, si_meta = exporter35.exporter34.exporter33.exporter32.export_si(
