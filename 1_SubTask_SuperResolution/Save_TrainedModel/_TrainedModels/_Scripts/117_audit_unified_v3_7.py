#!/usr/bin/env python
"""Strict additive 25-point audit for the mixed-resolution V3-7 figure.

V3-7 is a style-only refinement of the validated V3-6 figure.  This audit
retains the scientific/source checks and adds V3-7-specific geometry checks, then rechecks the
...
Running it writes only ``qa_v3_7.json`` in the supplied release directory.

The V3-4 audit is loaded for its tested hash/CSV/cache/PDF helpers.  V3-7
...
V3_6_BASELINE_ROOT = GENERATED_DIR / "MixedResolution_unified_v3_6_20260914_1129"
V3_6_BASELINE_PATHS = {
    V3_6_BASELINE_ROOT / "MixedResolution_unified_v3_6_20260914_1129.pdf":
        "7756662eed6beecb490eb28b663598f8e039fe08943c168e1b7b7772335ebefe",
    V3_6_BASELINE_ROOT / "MixedResolution_unified_v3_6_20260914_1129.svg":
        "3f0c974477979d8154a370dfac54e156165c492400854adaab2a8668a7fd38fa",
    V3_6_BASELINE_ROOT / "MixedResolution_unified_v3_6_20260914_1129.png":
        "ea4bf5dcc118b196b0aaf28fd1843e35afaa3cdd9402f6231ab1878d2724cc4a",
    V3_6_BASELINE_ROOT / "source_manifest_v3_6.json":
        "3785210e918bcdfa9627c7d3f4e8fe2a9ac0b6ef35da9ca1304764f791112dea",
    V3_6_BASELINE_ROOT / "qa_v3_6.json":
        "91046f7078b05921270083d3b525f83c58c397ce907f8c6a2d5009e3c9021acb",
}
...
def _normalise_row_boundaries(value):
    ...
    V3-7 metadata may store three [bottom, height] rows, three [bottom, top]
...
def _row_alignment(manifest, c, d):
    ...
    nested = _first(qa, "v3_7_row_boundaries_mm", "v3_6_row_boundaries_mm",
                    "v3_5_row_boundaries_mm", "row_boundaries_mm",
    ...
    if c_bounds is None:
        c_bounds = _normalise_row_boundaries(_first(
            qa, "v3_7_c_row_boundaries_mm", "v3_5_c_row_boundaries_mm", "c_row_boundaries_mm", default=None
        ))
    if d_bounds is None:
        d_bounds = _normalise_row_boundaries(_first(
            qa, "v3_7_d_row_boundaries_mm", "v3_5_d_row_boundaries_mm", "d_row_boundaries_mm", default=None
        ))
    if c_bounds is None:
        c_bounds = _normalise_row_boundaries(_first(
...
def _baseline_records(manifest, release):
    ...
    for path, digest in V3_5_BASELINE_PATHS.items():
        if path.exists():
            records.append({"path": str(path), "sha256": digest})
    for path, digest in V3_6_BASELINE_PATHS.items():
        if path.exists():
            records.append({"path": str(path), "sha256": digest})
...
def _find_main_file(release: Path, run_id: str, suffix: str):
    exact = release / f"MixedResolution_unified_v3_7_{run_id}.{suffix}"
    if exact.exists():
        return exact
    candidates = sorted(release.glob(f"MixedResolution_unified_v3_7_*_{run_id}.{suffix}"))
    if not candidates:
        candidates = sorted(release.glob(f"MixedResolution_unified_v3_7_*.{suffix}"))
...
def _baseline_v3_6_manifest():
    path = V3_6_BASELINE_ROOT / "source_manifest_v3_6.json"
    return _load_json(path), path
...
def main():
    ...
    parser.add_argument(
        "--manifest", type=Path,
        help="Optional source_manifest_v3_7.json; defaults to release-dir/source_manifest_v3_7.json",
    )
    args = parser.parse_args()
    release = args.release_dir.resolve()
    manifest_path = (args.manifest or (release / "source_manifest_v3_7.json")).resolve()
    qa_path = release / "qa_v3_7.json"
    checks = []
    ...
    # ------------------------------------------------------------------
    # V3-7 contract items 1--6: panel a.
    # ------------------------------------------------------------------
    ...
    check(2, "panel_a_explicit_rigid_blank_spacer", panel_a_spacer)
    ...
    # ------------------------------------------------------------------
    # V3-7 contract items 7--10: panel b.
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # V3-7 contract items 11--16: panels c/d and shared row geometry.
    # ------------------------------------------------------------------
    def shared_cd_grid():
        qa = _first(manifest, "cross_panel_alignment_qa", "panel_cd_alignment_qa", default={})
        qa = qa if isinstance(qa, dict) else {}
    ...
    # ------------------------------------------------------------------
    # V3-7 contract items 17--23: panel e.
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # V3-7 contract items 24--25: global scientific integrity/typography.
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    check(26, "additive_v3_7_schema_and_five_panel_narrative", lambda: (
        manifest.get("workflow_label") == "mixed_resolution_unified_v3_7"
        and str(manifest.get("schema_version")) == "3.7"
        ...
        and bool(manifest.get("v3_6_starting_renderer_anchor")
                 or manifest.get("v3_6_baseline")
    ...
    def documentation():
        required = ("quantitative_figure_report_v3_7.md", "figure_reference_update_v3_7.md",
                    "completion_report_v3_7.md")
        ...
        return (
            all(present.values()) and all(token in report_lower for token in ("v3-6", "matrix", "line plots"))
            and any(token in report_lower for token in ("no new evidence", "scientific content", "layout")),
            {"required": present},
        )

    check(35, "required_v3_7_documentation_complete", documentation)


    # ------------------------------------------------------------------
    # V3-7 anchor and targeted geometry checks.
    # ------------------------------------------------------------------
    ...
    def unchanged_scientific_values_and_sources():
        baseline, baseline_path = _baseline_v3_6_manifest()
        if not baseline:
        ...
        field_map = {
            "a": ("dimensions", "recipe_order", "exposure_values"),
    ...
    check(37, "scientific_values_and_validated_sources_unchanged_vs_v3_6",
    ...
    check(38, "c_d_shared_three_row_boundaries_and_topology_unchanged_vs_v3_6",
    ...
    def panel_a_allocation_and_contours():
        ...
        increase = _first(
            contour, "increase_vs_v3_6_mm", "increase_vs_v3_5_mm", "gap_increase_mm",
        ...
        contour_ok = (
            ...
            and (
                ...
                or _first(contour, "increased_vs_v3_6", "increased_vs_v3_5", default=False) is True
            )
            and (
        ...
        return allocation_ok and contour_ok, {
            ...
            "contour": {
                ...
                "increase_vs_v3_6_mm": increase,
                "qa": contour,
                "baseline_manifest": str(baseline_path),
            },
        }

    check(39, "panel_a_balanced_content_allocation_and_increased_contour_gaps",
    ...
    def panel_e_recipe_gap_contract():
        ...
        return (
            ...
            and central_larger
            and (
                _flag(e, "within_metric_recipe_gaps_reduced_vs_v3_6",
                      "recipe_gaps_reduced_vs_v3_6")
    ...
    def v3_6_hash_anchor():
        ...
        for path, digest in V3_6_BASELINE_PATHS.items():
            ...
            if actual != digest:
                mismatches.append({"path": str(path), "expected": digest, "actual": actual})
        records = []
        for key in ("v3_6_baseline", "v3_6_starting_renderer_anchor", "v3_6_hash_anchor"):
            records.extend(v34._records_from(manifest.get(key)))
        recorded = {
            v34._resolve_path(item.get("path"), release).resolve(): item.get("sha256")
            for item in records
        ...
        expected = {path.resolve(): digest for path, digest in V3_6_BASELINE_PATHS.items()}
        recorded_ok = all(recorded.get(path) == digest for path, digest in expected.items())
        return not missing and not mismatches and recorded_ok, {
            "missing": missing, "mismatches": mismatches,
            "recorded_anchor_complete": recorded_ok,
            "baseline_release": str(V3_6_BASELINE_ROOT),
        }

    check(44, "v3_6_hash_anchor_intact", v3_6_hash_anchor)

    def panel_a_v3_7_style():
        scales = _first(a, "heatmap_physical_scale_vs_v3_6", default=[])
        margin = float(_first(a, "h_inset_to_training_label_margin_mm", default=-1.0))
        minimum = float(_first(a, "minimum_h_inset_to_training_label_margin_mm", default=4.0))
        shift = _first(a, "roi_shift_parent_fraction", default=[])
        gap = float(_first(a, "contour_gap_mm", default=0.0))
        neighbor_clearances = list(map(float, _first(
            a, "inset_to_adjacent_heatmap_clearances_mm", default=[]
        )))
        return (
            len(scales) == 2 and np.allclose(scales, [.9, .9], rtol=0.0, atol=1e-12)
            and gap > 2.86944
            and len(shift) == 2 and float(shift[0]) < -.125 and float(shift[1]) > .100
            and margin >= minimum >= 5.0
            and len(neighbor_clearances) == 2 and min(neighbor_clearances) >= 1.0
            and _qa_passed(_first(a, "strict_anti_collision_margin_qa", default={}), "passed")
            and _qa_passed(_first(a, "inset_neighbor_collision_qa", default={}), "passed")
            and a.get("heatmaps_scaled_to_90_percent") is True
            and a.get("roi_shift_further_up_left_vs_v3_6") is True
        ), {"scales": scales, "gap_mm": gap, "roi_shift": shift,
            "margin_mm": margin, "minimum_mm": minimum,
            "neighbor_clearances_mm": neighbor_clearances}

    check(45, "panel_a_90pct_maps_up_left_roi_and_rigid_margin", panel_a_v3_7_style)

    def panel_b_v3_7_style():
        styles = _first(b, "legend_handle_styles", default=[])
        widths = [float(item.get("linewidth_pt")) for item in styles]
        markers = [float(item.get("marker_size_pt")) for item in styles]
        offsets = list(map(float, _first(b, "dmfg_annotation_offsets_measured_pt", default=[])))
        return (
            len(styles) == 4
            and np.allclose(widths, [1.45, 1.15, 1.15, 1.15], rtol=0.0, atol=1e-12)
            and np.allclose(markers, [4.2] * 4, rtol=0.0, atol=1e-12)
            and len(offsets) == 5
            and np.allclose(offsets, [1.4] * 5, rtol=0.0, atol=.02)
            and float(_first(b, "dmfg_annotation_offset_spread_pt", default=1.0)) <= .02
            and float(_first(b, "dmfg_annotation_min_ci_clearance_pt", default=-1.0)) >= .40
            and _qa_passed(_first(b, "legend_style_sync_qa", default={}), "passed")
            and _qa_passed(_first(b, "dmfg_annotation_alignment_qa", default={}), "passed")
        ), {"legend_styles": styles, "annotation_offsets_pt": offsets,
            "minimum_ci_clearance_pt": _first(b, "dmfg_annotation_min_ci_clearance_pt")}

    check(46, "panel_b_legend_trace_sync_and_uniform_bar_label_offset", panel_b_v3_7_style)

    def panel_e_v3_7_style():
        baseline, baseline_path = _baseline_v3_6_manifest()
        baseline_e = baseline.get("panels", {}).get("e", {}) if baseline else {}
        current_gaps = _metric_gap_values(e); old_gaps = _metric_gap_values(baseline_e)
        current_widths = list(map(float, _first(e, "matrix_cell_widths_mm", default=[])))
        baseline_bounds = _first(baseline_e, "metric_recipe_axes_bounds", default={})
        baseline_widths = []
    ...
    def panel_c_d_v3_7_style():
        ...
        alignment = _first(manifest, "cross_panel_alignment_qa", default={})
        pixel_qa = _first(alignment, "bottom_row_export_pixel_alignment", default={})
        return (
            len(widths) == 5 and np.allclose(widths, [.75] * 5, rtol=0.0, atol=1e-12)
            and _qa_passed(_first(c, "zoom_border_uniform_qa", default={}), "passed")
            and _marker_count(svg["c"], "panel-c-zoom-border") == 5
            and _qa_passed(pixel_qa, "passed")
            and int(_first(pixel_qa, "delta_pixels", default=-1)) == 0
            and float(_first(alignment, "max_boundary_delta_mm", default=1.0)) <= 1e-9
        ), {"border_widths_pt": widths, "pixel_alignment": pixel_qa,
            "max_boundary_delta_mm": _first(alignment, "max_boundary_delta_mm")}

    check(48, "panel_c_uniform_zoom_borders_and_c_d_bottom_pixel_alignment",
          panel_c_d_v3_7_style)

    passed = bool(checks) and all(item["passed"] for item in checks)
    payload = {
        "workflow_label": "mixed_resolution_unified_v3_7",
        "schema_version": "3.7",
        "run_id": args.run_id,
