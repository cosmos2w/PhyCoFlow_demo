#!/usr/bin/env python
"""Independent audit for the frozen-data mixed-resolution V4_2 bundle."""
...
V4_1_RELEASE = DEFAULT_RELEASE_ROOT / "MixedResolution_unified_v4_1_20260914_2000"
...
base = _load("mixed_resolution_v4_2_audit_helpers", HERE / "120_audit_mixed_resolution_v4.py")
...
def _source_lock_check(lock: dict, release: Path) -> tuple[bool, dict]:
    keys = ("v4_2_renderer", "v4_2_exporter", "v4_2_audit",
            "v4_2_panel_renderer", "v4_2_layout", "v4_2_wrapper")
    detail, passed = {}, True
    for key in keys:
        item = lock.get(key, {})
...
def audit(release_dir: Path, output: Path | None = None) -> dict:
    ...
    manifest = _read(release / "source_manifest_v4_2.json")
    ...
    baseline = _read(V4_1_RELEASE / "source_manifest_v4_1.json")
    pdfs = sorted(release.glob("MixedResolution_unified_v4_2_*.pdf"))
    svgs = sorted(release.glob("MixedResolution_unified_v4_2_*.svg"))
    pngs = sorted(release.glob("MixedResolution_unified_v4_2_*.png"))
    ...
    add("v4_2_release_identity_and_bookkeeping",
        required_ok and manifest.get("revision") == "V4_2" and release.name.startswith("MixedResolution_unified_v4_2_"),
    ...
    comparison_pass = (comparison.get("status") == "PASS"
                       ...
                       and comparison.get("v4_2_raw_or_scientific_deltas") is False
                       and bool(comparison_checks) and all(comparison_checks.values()))
    add("underlying_saved_data_exact_to_v4_1", comparison_pass,
    ...
    add("v4_2_panel_and_artist_inventory", inventory_pass, inventory)
    ...
    add("panel_a_tick_sized_resolution_captions", a_pass,
    ...
    b = panels.get("b", {}); baseline_b = baseline.get("panels", {}).get("b", {})
    ...
    b_pass = (b.get("top_y_label") == r"Relative $L_2$"
              ...
              and b.get("grouped_bar_axis_height_mm", 0) >= baseline_b.get("grouped_bar_axis_height_mm", 16.865) + 2.0
    ...
    add("panel_b_single_line_labels_height_and_flush_right_edge", b_pass,
    ...
    add("panel_d_multiscale_encoding_contours_and_tag_alignment", d_pass,
    ...
    add("panel_e_preserved_geometry_and_centering", e_pass,
    ...
    output_pass, output_detail = base._output_check(manifest, release, files, 180.0, 229.4, 600.0)
    add("compact_canvas_and_export_bundle", output_pass, output_detail)

    passed = all(item["passed"] for item in checks)
    payload = {"workflow_label": "mixed_resolution_unified_v4_2_audit",
               "schema_version": "4.2", "revision": "V4_2",
               "status": "PASS" if passed else "FAIL", "passed": passed,
               "release_dir": str(release), "checks": checks}
    destination = Path(output).resolve() if output else release / "qa_v4_2.json"
