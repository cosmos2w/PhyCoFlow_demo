#!/usr/bin/env python
"""Assemble the frozen-data mixed-resolution Figure V4_2 refinement."""
...
from common.publication_panels_unified_v4_2 import draw_panel, panel_label
...
v4_1 = _load("mixed_resolution_v4_1_base_for_v4_2", HERE / "121_assemble_mixed_resolution_unified_v4_1.py")
v4 = v4_1.v4
base = v4_1.base
V4_1_BASELINE = (
    HERE.parents[2] / "figures" / "generated" / "art_style_review"
    / "MixedResolution_unified_v4_1_20260914_2000"
)


def _v4_1_baseline_anchor() -> dict:
    paths = {
        "release_pdf": V4_1_BASELINE / "MixedResolution_unified_v4_1_20260914_2000.pdf",
        "release_svg": V4_1_BASELINE / "MixedResolution_unified_v4_1_20260914_2000.svg",
        "release_png": V4_1_BASELINE / "MixedResolution_unified_v4_1_20260914_2000.png",
        "release_manifest": V4_1_BASELINE / "source_manifest_v4_1.json",
        "release_qa": V4_1_BASELINE / "qa_v4_1.json",
        "renderer": HERE / "121_assemble_mixed_resolution_unified_v4_1.py",
        "exporter": HERE / "122_export_unified_v4_1_panels.py",
        "audit": HERE / "123_audit_mixed_resolution_v4_1.py",
        "panel_renderer": HERE / "common" / "publication_panels_unified_v4_1.py",
        "layout": HERE / "publication_layout_unified_v4_1.yaml",
    }
    missing = [str(path) for path in paths.values() if not path.exists()]
    if missing:
        raise FileNotFoundError(f"V4_1 baseline anchor is incomplete: {missing}")
    return {key: v4._record(path) for key, path in paths.items()}


def _geometry_v4_2(layout):
    """Use an explicit 3-mm c/d-to-e gap without altering other major gaps."""
    ...
    if abs(c_width + spacer + d_width - width) > 1e-9:
        raise ValueError("V4_2 c/spacer/d widths do not close to the canvas width")
    total = sum(heights.values()) + sum(gaps.values())
    if abs(total - height) > 1e-9:
        raise ValueError(f"V4_2 row heights and gaps close to {total}, not {height} mm")
    e_bottom = 0.0
...
def _create_canvas_v4_2(layout):
    previous = v4._geometry
    v4._geometry = _geometry_v4_2
    try:
...
def _create_standalone_canvas_v4_2(layout, label):
    previous = v4._geometry
    v4._geometry = _geometry_v4_2
    try:
...
def _author_delta_contract(panel_meta: dict) -> dict:
    return {
        "authorization": "explicit user instructions in V4_1 and V4_2 revision tasks",
        ...
        "v4_2": {
            ...
            "scope": "colormap lookup, contour styling, typography, and geometry only",
        },
    }
...
def main():
    ...
    parser.add_argument("--layout", type=Path, default=HERE / "publication_layout_unified_v4_2.yaml")
    ...
    style_record["scope"] = "V4_2 process-local override; V4_1 and scientific sources untouched"
    ensure_output_dirs()
    layout = base.load_layout(layout_path)
    ctx = base.make_context(args, cfg, layout, rid)
    v3_anchor = v4._v3_7_baseline_anchor()
    v4_1_anchor = _v4_1_baseline_anchor()
    results_before = v4._tree_state(RESULTS_DIR)
    out = FIGURES_DIR / "Assembled" / f"MixedResolution_unified_v4_2_{rid}"
    manifest_path = FIGURES_DIR / "Assembled" / f"FigureSourceManifest_unified_v4_2_{rid}.json"
    targets = [out.with_suffix(ext) for ext in (".svg", ".pdf", ".png")] + [manifest_path]
    if any(path.exists() for path in targets):
        raise FileExistsError(f"Refusing to overwrite existing V4_2 artifacts for {rid}")

    fig, axes, containers, rects, width, height, shared_axes, cbar_parent = _create_canvas_v4_2(layout)
    panel_meta = {}
    for label in ("a", "b"):
    ...
    if not model_artist_qa["passed"]:
        raise ValueError(f"V4_2 model artist contract failed: {model_artist_qa}")
    if not geometry_qa["passed"]:
        raise ValueError(f"V4_2 geometry failed: {geometry_qa}")
    if not alignment_qa["passed"]:
        raise ValueError(f"V4_2 shared c+d alignment failed: {alignment_qa}")
    qa = {
    ...
    if results_before != results_after:
        raise RuntimeError("Validated _Process_Results changed during V4_2 rendering")
    ...
    manifest.update({
        "workflow_label": "mixed_resolution_unified_v4_2",
        "schema_version": "4.2",
        "revision": "V4_2",
        "source_visual_revision": "V4_1",
        "v4_1_baseline_anchor": v4_1_anchor,
        "author_authorized_deltas": _author_delta_contract(panel_meta),
        "renderer": v4._record(Path(__file__).resolve()),
        "panel_renderer": v4._record(HERE / "common" / "publication_panels_unified_v4_2.py"),
        "exporter": v4._record(HERE / "125_export_unified_v4_2_panels.py"),
    })
    ...
    manifest["figure_contract"].update({
        "core_conclusion": (
            ...
            "relative fidelity; V4_2 gives the multi-scale component and error evidence distinct semantics."
        ),
        "revision_scope": (
            "V4_2 aesthetic refinement using unchanged saved fields and metrics; V4_1's explicit "
            "ROI, visible-annotation, and sweep-inventory exceptions remain recorded."
        ),
