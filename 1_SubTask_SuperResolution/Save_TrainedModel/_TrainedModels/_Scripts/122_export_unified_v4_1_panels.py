#!/usr/bin/env python
"""Export standalone V4_1 panels and preserve the validated V4 SI bundle."""
...
from common.publication_panels_unified_v4_1 import PANEL_OUTPUT_NAMES, draw_panel, panel_label
...
assembler = _load("unified_v4_1_assembler_for_export", HERE / "121_assemble_mixed_resolution_unified_v4_1.py")
exporter37 = _load("unified_v3_7_exporter_for_v4_1", HERE / "116_export_unified_v3_7_panels.py")
...
def export_standalone_panels(ctx, cfg, layout, root: Path, rid: str):
    ...
    for label in "abcde":
        fig, ax, _container = assembler.v4.create_standalone_canvas(layout, label)
...
def main():
    ...
    rid = args.run_id
    if not rid:
        raise ValueError("--run-id is required for additive V4_1 exports")
    ...
    root = FIGURES_DIR / "UnifiedV4_1" / rid
    if root.exists():
        raise FileExistsError(f"Refusing to overwrite existing V4_1 export directory: {root}")
    root.mkdir(parents=True)
    panel_outputs, panel_meta = export_standalone_panels(ctx, cfg, layout, root, rid)

    for name in ("si", "tables"):
        source = V4_RELEASE / name
        if not source.is_dir():
            raise FileNotFoundError(f"Validated V4 {name} directory is missing: {source}")
    ...
    manifest["si_outputs"] = {
        "preserved_v4": [base.record(path) for path in sorted((root / "si").rglob("*")) if path.is_file()]
    }
    manifest["si_metadata"] = {
        "source_revision": "V4", "restyled": False,
        "reason": "SI is outside the active V4_1 main-figure scope",
        "source_directory": str((V4_RELEASE / "si").resolve()),
    }
    manifest["table_outputs"] = {
        "preserved_v4": [base.record(path) for path in sorted((root / "tables").rglob("*")) if path.is_file()]
    }
    v4_manifest = json.loads((V4_RELEASE / "source_manifest_v4.json").read_text(encoding="utf-8"))
    for key in (
