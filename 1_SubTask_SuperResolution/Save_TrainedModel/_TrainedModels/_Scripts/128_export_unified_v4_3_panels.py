#!/usr/bin/env python
"""Export standalone V4_3 panels and preserve the validated V4_2 SI bundle."""
...
assembler = _load("unified_v4_3_assembler_for_export", HERE / "127_assemble_mixed_resolution_unified_v4_3.py")
exporter37 = _load("unified_v3_7_exporter_for_v4_3", HERE / "116_export_unified_v3_7_panels.py")
...
def export_standalone_panels(ctx, cfg, layout, root: Path, rid: str):
    ...
    for label in "abcde":
        fig, ax, _container = assembler._create_standalone_canvas_v4_3(layout, label)
...
def main():
    ...
    rid = args.run_id
    if not rid:
        raise ValueError("--run-id is required for additive V4_3 exports")
    ...
    root = FIGURES_DIR / "UnifiedV4_3" / rid
    if root.exists():
        raise FileExistsError(f"Refusing to overwrite existing V4_3 export directory: {root}")
    root.mkdir(parents=True)
    panel_outputs, panel_meta = export_standalone_panels(ctx, cfg, layout, root, rid)

    for name in ("si", "tables"):
        source = V4_2_RELEASE / name
        if not source.is_dir():
            raise FileNotFoundError(f"Validated V4_2 {name} directory is missing: {source}")
    ...
    manifest["si_outputs"] = {
        "preserved_v4_2": [base.record(path) for path in sorted((root / "si").rglob("*")) if path.is_file()]
    }
    manifest["si_metadata"] = {
        "source_revision": "V4_2", "restyled": False,
        "reason": "SI is outside the active V4_3 main-figure scope",
        "source_directory": str((V4_2_RELEASE / "si").resolve()),
    }
    manifest["table_outputs"] = {
        "preserved_v4_2": [base.record(path) for path in sorted((root / "tables").rglob("*")) if path.is_file()]
    }
    v4_manifest = json.loads((V4_2_RELEASE / "source_manifest_v4_2.json").read_text(encoding="utf-8"))
    for key in (
