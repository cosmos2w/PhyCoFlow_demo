...
def main() -> None:
    ...
    wave_meta_path = RESULTS / "MultiscaleWavelet" / f"MultiscaleWavelet_metadata_{args.run_id}.json"
    data_manifest_path = RESULTS / "UnifiedPublicationV2" / f"UnifiedV2DataManifest_{args.run_id}.json"
    ...
    sweep_path = RESULTS / "UnifiedPublicationV2" / f"SensorSweepAllRecipes_summary_{args.run_id}.csv"
    accuracy_path = RESULTS / "UnifiedPublicationV2" / f"AllRecipeAccuracy_summary_{args.run_id}.csv"
    wavelet_path = RESULTS / "MultiscaleWavelet" / f"MultiscaleWavelet_summary_{args.run_id}.csv"
    wavelet_per_path = RESULTS / "MultiscaleWavelet" / f"MultiscaleWavelet_per_snapshot_{args.run_id}.csv"
    ...
    snapshot_path = RESULTS / "SensorSweep" / f"SensorSweep_per_snapshot_{args.base_data_run_id}.csv"
    ...
    for model in MODELS:
        ...
        paired_contrasts.append([
            model_labels[model],
            f"{pct_change(mixed, hlim):+.1f}%",
            f"{pct_change(rich, balanced):+.1f}%",
        ])
    ...
    add([
        ...
        f"- Final canvas: {canvas_w:.1f} × {canvas_h:.3f} mm.",
        ...
        "",
        "The refreshed checkpoint comparison strengthens the global-error result for DMF-Gen, Senseiver, and MLP-RBF, while exposing an important checkpoint-level tradeoff: DMF-Gen Zero-H-M-rich remains better than Zero-H-balanced at 512 sensors (0.0288 versus 0.0334), but its own value is 6.6% worse than in the supplied July baseline. Accordingly, the paper should emphasize DMF-Gen's zero-H robustness and fine-scale preservation rather than imply that every refreshed checkpoint improved.",
        "",
        ...
        "",
        "The key interaction is architecture dependence. Relative to H-limited training, Mixed-HML strongly lowers error for FFM-Perceiver, modestly lowers it for DMF-Gen and MLP-RBF, and increases it for Senseiver. DMF-Gen remains the lowest-error zero-H model, and its M-rich recipe lowers error by 13.7% relative to balanced zero-H in the refreshed run.",
        "",
        f"Primary table: [`SensorSweepAllRecipes_summary_{args.run_id}.csv`](../../_Process_Results/UnifiedPublicationV2/SensorSweepAllRecipes_summary_{args.run_id}.csv).",
        "",
        ...
        "",
        "Panel d contains 100 aggregate cells (5 recipes × 4 models × 5 counts), each based on 300 paired case–time errors. All axes share the same log-scale range.",
        "",
        ...
        "",
        "Panel f summarizes 4 models × 3 displayed recipes × 300 snapshots × 3 scales. Its upper heatmap is median spatial pattern correlation; its lower heatmap is median variance-allocation bias in percentage points. Each aggregate cell has `valid_n=300`.",
        "",
        ...
        "",
        f"Primary table: [`MultiscaleWavelet_summary_{args.run_id}.csv`](../../_Process_Results/MultiscaleWavelet/MultiscaleWavelet_summary_{args.run_id}.csv).",
        "",
        ...
        *[
        ...
        "",
        "We first compared five training-resolution recipes while holding the H-resolution evaluation cohort and nested sensor plans fixed (Fig. a–d). Mixed-resolution training had architecture-dependent effects. At 512 sensors, Mixed-HML reduced mean physical relative L2 relative to H-limited training by 8.0%, 43.1%, and 6.9% for DMF-Gen, FFM-Perceiver, and MLP-RBF, respectively, but increased Senseiver error by 74.5%. Thus, lower-resolution data were not uniformly beneficial; their value depended on how the reconstruction architecture integrated multiresolution training examples.",
        "",
        f"DMF-Gen was the most robust model when H-resolution training cases were absent. Its mean error at 512 sensors was {summary_row(sweep, 'DMFGen', '4_ZeroH_Balanced', 512)['mean']:.4f} for Zero-H-balanced and {summary_row(sweep, 'DMFGen', '5_ZeroH_MRich', 512)['mean']:.4f} for Zero-H-M-rich, lower than the corresponding non-DMF errors. The advantage persisted across the 64–512 sensor sweep, indicating that it was not confined to the highest observation budget. The refreshed checkpoints improve DMF-Gen's five-recipe average 512-sensor error by {abs(float(change_rows[0][3].rstrip('%'))):.1f}% relative to the supplied baseline. Within the refreshed run, M-rich lowers DMF-Gen error by 13.7% relative to balanced zero-H, even though the refreshed M-rich checkpoint itself is 6.6% worse than its July baseline value.",
        "",
        ...
        "### Caption draft",
        "",
        "**Mixed-resolution training and multiscale fidelity in sparse H-resolution field reconstruction.** **a,** Native L-, M-, and H-resolution density fields and the five training-resolution recipes; bars report active training cases and labels give spatial degree-of-freedom budgets relative to H-only. **b,** Mean physical relative L2 error at 512 nested H-grid sensors for four reconstruction models and five training recipes (n=300 held-out case–time pairs; error bars, bootstrap 95% confidence intervals). **c,** Matched H-resolution reconstructions, ground-truth-selected zoom, and pointwise absolute errors for one configured held-out snapshot; all fields and errors use shared normalization. **d,** Mean physical relative L2 across 64–512 nested sensors for each training recipe (n=300 per point). **e,** Ground-truth wavelet components and signed model residuals for DMF-Gen and the best non-DMF Zero-H-M-rich baseline at 256 sensors; rows show large, intermediate, and fine scales, with row-specific symmetric display limits. **f,** Median spatial pattern correlation and variance-allocation bias across 300 held-out fields for Mixed-HML and both zero-H recipes. Lower L2 and variance bias closer to zero are better; higher correlation is better.",
        "",
        ...
        f"- Unified data manifest: [UnifiedV2DataManifest_{args.run_id}.json](../../_Process_Results/UnifiedPublicationV2/UnifiedV2DataManifest_{args.run_id}.json).",
        f"- All-recipe accuracy: [AllRecipeAccuracy_summary_{args.run_id}.csv](../../_Process_Results/UnifiedPublicationV2/AllRecipeAccuracy_summary_{args.run_id}.csv).",
        f"- All-recipe sensor sweep: [SensorSweepAllRecipes_summary_{args.run_id}.csv](../../_Process_Results/UnifiedPublicationV2/SensorSweepAllRecipes_summary_{args.run_id}.csv).",
        f"- Multiscale summary: [MultiscaleWavelet_summary_{args.run_id}.csv](../../_Process_Results/MultiscaleWavelet/MultiscaleWavelet_summary_{args.run_id}.csv).",
        f"- Multiscale per-snapshot table: [MultiscaleWavelet_per_snapshot_{args.run_id}.csv](../../_Process_Results/MultiscaleWavelet/MultiscaleWavelet_per_snapshot_{args.run_id}.csv).",
        f"- Multiscale metadata: [MultiscaleWavelet_metadata_{args.run_id}.json](../../_Process_Results/MultiscaleWavelet/MultiscaleWavelet_metadata_{args.run_id}.json).",
