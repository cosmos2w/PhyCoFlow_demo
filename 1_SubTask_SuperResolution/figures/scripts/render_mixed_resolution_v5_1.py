#!/usr/bin/env python
"""Reflow the frozen V4_7 mixed-resolution evidence as six-panel V5_1.

This renderer reads the existing publication tables and cached panels. The
changes from V5_0 are axes positions, compact tick labels, and shared recipe
labels. It
does not train a model or infer a field. The inherited qualitative
...
LAYOUT_PATH = SCRIPTS / "publication_layout_unified_v5_1.yaml"
BASELINE_DIR = (
...
V5_0_PDF = V5_0_DIR / "MixedResolution_unified_v5_0_20260926_1336.pdf"
...
v47 = _load_module("mixed_resolution_v4_7_for_v5_1", SCRIPTS / "139_assemble_mixed_resolution_unified_v4_7.py")
base = v47.base
v4 = v47.v4
...
def _reflow_a(parent, geo: dict, width: float, height: float) -> dict:
    ...
    # The five recipe bars share the exact x positions with panel b. Keeping
    # the recipe labels once, under b, saves a full text row without losing
    # category identity; the exposure values above a remain visible.
    bar.set_xlabel("")
    bar.tick_params(axis="x", labelbottom=False)
...
def _reflow_b_c(parent, geo: dict, ctx, width: float, height: float) -> dict:
    ...
    if len(sweeps) != 3:
        raise RuntimeError("V5_1 requires all three saved sensor sweeps")
    ...
    # title and crowds the shorter V5_1 bar axis.
    for item in list(bar.texts):
        if item.get_gid() == "panel-b-inside-title":
            item.remove()
    bar.set_title("High-resolution reconstruction (512 sensors)", pad=1.5)
    recipe_abbreviations = ["H-only", "H-limited", "Mixed-HML", "Zero-H bal.", "Zero-H M-rich"]
    bar.set_xticks(range(5), recipe_abbreviations)
    ...
    return {
        ...
        "recipe_labels": recipe_abbreviations,
        "sweep_count": len(sweeps),
    }


def _compact_f_scale_ticks(parent) -> dict:
    """Use a shorter middle-scale label after the inherited source audit."""
    ...
    for axis in axes:
        axis.set_xticks(axis.get_xticks(), ["Large", "Mid", "Fine"])
    ...
    return {"matrix_axis_count": len(axes), "displayed_scale_labels": ["Large", "Mid", "Fine"],
            "mid_abbreviation_expands_to": "Intermediate"}
...
def _publication_panel_metadata(drawn: dict) -> dict:
    """Store V5_1 panel roles without stale V4_7 cross-panel geometry fields."""
    ...
    for label, panel in panels.items():
        panel.update({
            "panel_id": f"fig3.{label}", "figure_revision": "V5_1",
...
def main() -> None:
    ...
    if args.trial_dir:
        ...
        release = ROOT / "figures" / "generated" / "art_style_review" / f"MixedResolution_unified_v5_1_{rid}"
    if release.exists():
    ...
    collision = measure_annotation_collision_gate(
        fig,
        minimum_clearance_mm=float(layout["v5_1_geometry_gate"]["minimum_annotation_data_clearance_mm_at_180mm"]),
        strict=False,
    )
    topology = _topology_qa(
        fig, axes, containers, shared, width, height,
        layout["v5_1_geometry_gate"],
    ...
    if not gates_passed and not args.trial_dir:
        raise RuntimeError("V5_1 scientific, typography, alignment, or layout gate failed")
    stem = release / f"MixedResolution_unified_v5_1_{rid}"
    ...
    if results_before != v4._tree_state(RESULTS_DIR):
        raise RuntimeError("Validated process results changed during V5_1 rendering")
    if baseline_hash != _sha256(baseline_pdf):
        raise RuntimeError("V4_7 baseline PDF changed during V5_1 rendering")
    if v5_0_hash != _sha256(V5_0_PDF):
        raise RuntimeError("V5_0 baseline PDF changed during V5_1 rendering")
    previews = _previews(stem.with_suffix(".pdf"), stem.with_suffix(".png"), release)
    manifest = {
        "schema_version": "5.1", "revision": "V5_1", "run_id": rid,
        "workflow_label": "mixed_resolution_unified_v5_1",
    ...
    _write_json(release / "source_manifest_v5_1.json", manifest)
    ...
    _write_json(release / "LAYOUT_QA.json", {
        "revision": "V5_1", "tested_design_width_mm": width,
    ...
    _write_json(release / "SOURCE_LOCK.json", {
        "revision": "V5_1", "status": "RECORDED", "run_id": rid,
        "backend": "Python/Matplotlib in fig environment",
    ...
    (release / "figure_contract.md").write_text(
        f"# Mixed-resolution Figure V5_1 contract\n\n"
        ...
        f"- Panels a/b share five exactly aligned recipe positions. The recipe names appear once below b; "
        f"the five values above the a bars are relative spatial-field exposure. "
        f"'Zero-H bal.' abbreviates Zero-H-balanced.\n"
        f"- Matrix scale labels: 'Mid' abbreviates Intermediate; Large and Fine retain "
        f"their V5_0 meanings.\n"
        f"- Artist-union row gaps are {topology['vertical_artist_gaps_mm']} mm at 180 mm, "
        f"equivalent to {topology['vertical_gap_fraction_of_v5_0']} of V5_0.\n"
        f"- Status: **art reviewed, scientific release pending** (existing A03/A04 author decisions).\n",
        encoding="utf-8",
    )
    (release / "STYLE_CHANGELOG.md").write_text(
        f"# V5_0 → V5_1 visual changes\n\n"
        f"- Reduced canvas height from 210.0 to {height:.1f} mm "
        f"({topology['height_reduction_percent']:.1f}%).\n"
        f"- Compressed each major row gap to {topology['vertical_gap_fraction_of_v5_0']} "
        f"of its V5_0 rendered artist gap.\n"
        f"- Shared the five recipe x positions across a/b and kept their names once below b.\n"
        f"- Tightened the three sensor-sweep intervals while retaining all data windows.\n"
        f"- Reallocated d/e grid margins and row gutters, and tightened f's title/heatmap bands.\n"
        f"- Shortened only the f matrix tick 'Intermediate' to 'Mid'; numeric cells are unchanged.\n"
    ...
    if not args.trial_dir:
        ...
        for ext in (".pdf", ".svg", ".png"):
            target = assembled / f"MixedResolution_unified_v5_1_{rid}{ext}"
            if target.exists():
                raise FileExistsError(target)
            shutil.copy2(stem.with_suffix(ext), target)
        target = assembled / f"FigureSourceManifest_unified_v5_1_{rid}.json"
        if target.exists():
            raise FileExistsError(target)
        shutil.copy2(release / "source_manifest_v5_1.json", target)
