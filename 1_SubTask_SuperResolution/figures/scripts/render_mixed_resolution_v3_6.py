#!/usr/bin/env python
"""Build, package, document, and audit the additive Figure V3-6 release."""
...
RESULTS = MODELS_ROOT / "_Process_Results"; LAYOUT = SCRIPTS / "publication_layout_unified_v3_6.yaml"
...
# Mixed-resolution Figure V3-6 contract

- Core conclusion: V3-6 preserves the validated conclusion that L/M/H are distinct spatial discretizations and that DMF-Gen retains the strongest H-resolution fidelity as H-resolution training fields are removed.
...
- Revision scope: final micro-adjustments to aspect ratios, whitespace, and marker styling only; no scientific redesign, source substitution, metric recomputation, retraining, or inference.
...
- V3-6 preserves the V3-5 shared c+d structural grid exactly while rebalancing panel a, opening panel b vertically, and condensing panel e.
""")
    write(release / "figure_reference_update_v3_6.md", """
# Figure-reference update — V3-5 to V3-6

| V3-5 reference | V3-6 reference | Updated role |
|---|---|---|
| Fig. 3a | Fig. 3a | The validated L/M/H context and recipe budgets remain; contour/bar allocation is rebalanced to approximately 45:55 and contour spacing is opened. |
| Fig. 3b, upper | Fig. 3b, upper | The grouped transfer bars retain identical values with a 1.155× taller plotting axis. |
| Fig. 3b, lower | Fig. 3b, lower | The zero-H sweeps retain identical values, use taller axes, thinner lines, and larger markers. |
| Fig. 3c | Fig. 3c | Unchanged from V3-5, including exact shared three-row geometry with panel d. |
| Fig. 3d | Fig. 3d | Unchanged from V3-5; Large/Intermediate/Fine qualitative decomposition remains qualitative-only. |
| Fig. 3e | Fig. 3e | Recipe gaps are condensed, the central metric split is emphasized, colorbar labels are removed, and the bars move closer to the matrices. |
...
""")
    write(release / "quantitative_figure_report_v3_6.md", f"""
# Quantitative figure report — V3-6

V3-6 is a bounded, layout-only micro-adjustment release with no new evidence. It preserves the V3-5 structural grid and every validated matrix value. The redundant fine-scale line plots remain removed from the main figure and retained through panel e and SI. No quantitative value, source selection, representative state, model cache, or scientific claim was changed.
...
- V3-6 increases the three plotting-axis heights by 1.155×, while the two sweeps use thinner lines and larger markers.
...
- V3-6 preserves V3-5's shared exact row boundaries for panels c/d and keeps validated field/error scales and sensor-layout footprints.
...
- Distinct correlation and bias ranges remain; recipe gaps are narrower, the central metric gap remains larger, and the unlabeled colorbar band sits closer to the matrices.

## Provenance and SI

- Validated cache fields, summary tables, standalone SI figures, and LaTeX tables are copied or referenced only; source hashes are recorded in `source_manifest_v3_6.json`.
- The exact V3-5 renderer stack and main outputs are anchored by hash before assembly, and all V3-5/V3-4/V3-3/V3-2/V2 baseline artifacts are checked for immutability afterward.
- Training, model inference, and metric recomputation were not performed.
""")
    write(release / "completion_report_v3_6.md", f"""
# Mixed-resolution Figure V3-6 completion report
...
- V3-5 and earlier baseline artifacts unchanged: {manifest.get('baseline_immutability', {}).get('unchanged', 'pending')}.
...
- QA: the V3-6 audit contract, SI completeness, source hashes, baseline hashes, and direct cache checks are enforced by `114_audit_unified_v3_6.py`.
""")
...
def main():
    ...
    release = DELIVERY_ROOT / f"MixedResolution_unified_v3_6_{rid}"
    main_base = FIGURES / "Assembled" / f"MixedResolution_unified_v3_6_{rid}"
    manifest_source = FIGURES / "Assembled" / f"FigureSourceManifest_unified_v3_6_{rid}.json"
    process_root = FIGURES / "UnifiedV3_6" / rid
    ...
    if any(path.exists() for path in collisions):
        raise FileExistsError(f"Refusing to overwrite existing V3-6 artifacts: {collisions}")
    baseline_paths = [
    ...
    subprocess.run([sys.executable, str(SCRIPTS / "112_assemble_mixed_resolution_unified_v3_6.py"), *common],
                   cwd=SCRIPTS, check=True)
    subprocess.run([sys.executable, str(SCRIPTS / "113_export_unified_v3_6_panels.py"), *common],
                   cwd=SCRIPTS, check=True)
    after = tree_state(RESULTS)
    if before != after:
        raise RuntimeError("Validated _Process_Results changed during the visualization-only V3-6 build")
    baseline_after = {str(path): sha256(path) for path in baseline_paths}
    if baseline_before != baseline_after:
        raise RuntimeError("A V3-5, V3-4, V3-3, V3-2, or requested V2 baseline artifact changed during the V3-6 build")
    ...
    manifest["orchestration"] = {
        ...
        "panel_exporter": record(SCRIPTS / "113_export_unified_v3_6_panels.py"),
        "audit": record(SCRIPTS / "114_audit_unified_v3_6.py"),
    }
    ...
    source_manifest = release / "source_manifest_v3_6.json"
    ...
    subprocess.run([sys.executable, str(SCRIPTS / "114_audit_unified_v3_6.py"),
                    "--run-id", rid, "--release-dir", str(release)], cwd=SCRIPTS, check=True)
    qa = json.loads((release / "qa_v3_6.json").read_text(encoding="utf-8"))
    build_docs(release, manifest, qa_status="pass" if qa.get("passed") else "fail")
    if not qa.get("passed"):
        raise RuntimeError(f"V3-6 QA failed: {release / 'qa_v3_6.json'}")
    print(f"[OK] {release}")
