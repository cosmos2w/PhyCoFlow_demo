...
def make_standalone(panel: str, data: dict, config: dict):
    label = str(config["scenario"]["label"])
    if panel in {"a", "b"}:
    ...
    if panel == "c":
        draw_accuracy_cost(ax, data["cost_native"], config, title=f"{label} native accuracy–latency", xlabel="Warm model-core latency (ms)", panel_label="c", ylabel="Mean density relative L2", ylim=ylim)
    elif panel == "d":
        draw_accuracy_cost(ax, data["training_cost"], config, title=f"{label} accuracy–training update time", xlabel=data["training_metric_label"], panel_label="d", ylabel="Mean density relative L2", ylim=ylim)
    else:
...
def make_composed(data: dict, config: dict):
    ...
    ylim = _shared_log_error_limits(data["cost_native"], data["training_cost"])
    draw_accuracy_cost(ax_c, data["cost_native"], config, title=f"{label} native accuracy–latency", xlabel="Warm model-core latency (ms)", panel_label="c", ylabel="Mean density relative L2", ylim=ylim)
    draw_accuracy_cost(ax_d, data["training_cost"], config, title=f"{label} accuracy–training update time", xlabel=data["training_metric_label"], panel_label="d", ylabel=None, ylim=ylim)
