#!/usr/bin/env python
"""Export cache-only CH4--T joint-PDF JSD diagnostics.

The exporter reads finalized reconstruction caches and never loads a model or
regenerates predictions.  It defaults to the T-only condition used by Panel d,
while retaining an explicit ``--conditions`` option for reproducible extension.
"""
...
def _manifest_path(args) -> Path:
    ...
    exact = (
        RESULTS_DIR / "ReconstructionCache" / f"ReconstructionCache_manifest_{args.run_id}.csv"
        if args.run_id else None
    )
    return (
        exact if exact and exact.exists()
        else latest(RESULTS_DIR / "ReconstructionCache", "ReconstructionCache_manifest", "csv")
    )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Condition keys; defaults to the T-only Panel-d condition.",
    )
    ...
    cache_lookup = {
        (row["method"], row["condition"], int(row["snapshot"])): row for row in manifest
    }
    edges = global_edges(
        manifest, [PAIR], args.bins or cfg["defaults"]["pdf_bins"],
        cfg["defaults"]["robust_quantiles"],
    )[PAIR]
    ...
    for method in methods:
        for condition in conditions:
            for snapshot in expected[condition]:
                ...
                status, value, detail = entry.get("status", "missing cache"), np.nan, ""
                if status == "ok" and entry.get("cache_path"):
                    try:
                        arrays, _ = load_cache(Path(entry["cache_path"]))
                        first, second = PAIR_FIELDS[PAIR]
                        truth = histogram(
                            arrays["truth_phys"][:, first], arrays["truth_phys"][:, second], edges,
                        )
                        reconstruction = histogram(
                            arrays["recon_phys"][:, first], arrays["recon_phys"][:, second], edges,
                        )
                        value = jsd_base2(truth, reconstruction, cfg["defaults"]["pdf_pseudocount"])
                    except Exception as exc:
                        status, detail = "inference error", type(exc).__name__
                    else:
                        detail = "cache-only CH4-T"
                rows.append({
                    "run_id": rid, "method": method["name"], "condition": condition,
                    "snapshot": snapshot, "pair": PAIR, "jsd_base2": value,
                    "status": status, "value_source": "cache_CH4-T",
                    "checkpoint": entry.get("checkpoint_name", ""),
                    "family": entry.get("family", ""), "n_steps": entry.get("n_steps", ""),
                    "ode_solver": entry.get("ode_solver", ""), "detail": detail,
                })
    ...
    per_path = output / artifact_name("CH4TCouplingJSD_per_snapshot", rid, "csv")
    summary_path = output / artifact_name("CH4TCouplingJSD_summary", rid, "csv")
    edges_path = output / artifact_name("CH4TCouplingJSD_bin_edges", rid, "csv")
    ...
    print(f"[OK] cache-only CH4--T JSD: {per_path}\n[OK] {summary_path}\n[OK] {edges_path}")
    return 0
