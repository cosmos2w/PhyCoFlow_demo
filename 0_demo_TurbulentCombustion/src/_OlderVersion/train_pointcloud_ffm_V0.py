...
from helpers_V0 import MetricsLogger, TurbulentCombustionH5Dataset, create_recon_dir, visualize_reconstruction, build_sparse_condition
...
def parse_args():
    ...
    p.add_argument("--n-obs-min", type=int, default=64)
    p.add_argument("--n-obs-max", type=int, default=256)

    p.add_argument("--cond-field", type=int, default=2, help="0:CH4, 1:CO, 2:T, 3:U_1, 4:p")
...
class ConditionalPointFFM(nn.Module):
    def __init__(
        ...
        self.obs_encoder = make_mlp(coord_dim + 1, cond_dim, cond_dim, depth=3)
        self.global_encoder = make_mlp(hidden_dim, hidden_dim, hidden_dim, depth=2)
        self.head = nn.Sequential(
            nn.Linear(hidden_dim + hidden_dim + cond_dim + field_embed_dim, hidden_dim),
            nn.GELU(),
    ...
    def aggregate_sparse_obs(
        ...
        obs_feat = self.obs_encoder(torch.cat([obs_coords, obs_values], dim=-1))
        obs_feat = obs_feat * obs_mask.unsqueeze(-1)

        # d2 = ((query_coords.unsqueeze(2) - obs_coords.unsqueeze(1)) ** 2).sum(dim=-1)
        # --- OPTIMIZED DISTANCE CALCULATION ---
        # torch.cdist computes the Euclidean distance (B, N, M) directly. 
        # We square it to get the d2 for the RBF kernel.
    ...
    def forward(
        ...
        local_cond = self.aggregate_sparse_obs(coords, obs_coords, obs_values, obs_mask)
        global_feat = self.global_encoder(point_feat.mean(dim=1)).unsqueeze(1).expand(bsz, n_pts, -1)
        field_feat = self.field_embed(cond_field_idx).unsqueeze(1).expand(bsz, n_pts, -1)
        
        return self.head(torch.cat([point_feat, global_feat, local_cond, field_feat], dim=-1))
...
class PointCloudFFM(nn.Module):
    ...
    def training_loss(
        ...
        pred = self.model(t, x_t, coords, obs_coords, obs_values, obs_mask, cond_field_idx)
    ...
    def sample(
        ...
        for i in range(n_steps):
            ...
            v0 = self.model(t0, x, coords, obs_coords, obs_values, obs_mask, cond_field_idx)
            x_euler = x + dt * v0
            v1 = self.model(t1, x_euler, coords, obs_coords, obs_values, obs_mask, cond_field_idx)
            x = x + 0.5 * dt * (v0 + v1)
            if clamp_indices is not None:
                for b in range(bsz):
                    valid = obs_mask[b].bool()
                    idx = clamp_indices[b, valid]
                    fld = int(cond_field_idx[b].item())
                    x[b, idx, fld] = obs_values[b, valid, 0]
        return x
...
def run_epoch(
    ...
    for batch in pbar:
        ...
        cond_field_idx = torch.full((coords_full.shape[0],), cond_field, device=device, dtype=torch.long)

        obs_coords, obs_values, obs_mask, _ = build_sparse_condition(
            coords_full, fields_full, cond_field_idx, n_obs_min=n_obs_min, n_obs_max=n_obs_max
        )
...
def main():
    ...
    set_seed(args.seed)
    # demo_tag = f"_DemoN{args.Demo_Num}"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.save_every == 0:
            ...
            visualize_reconstruction(
                ...
                save_dir=recon_dir,
                cond_field=args.cond_field,
                n_obs=256,         # Number of sensors to visualize
                n_steps=args.n_steps_generation,       # Euler steps for generation
                snapshot_index=0   # Keeps the same fluid snapshot (from validation set) across epochs
            )
