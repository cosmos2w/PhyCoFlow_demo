...
- [4. Generative and flow reconstruction models](#4-generative-and-flow-reconstruction-models)
  ...
  - [4.3 PointCloudFFM](#43-pointcloudffm)
    - [GL-RBF enhanced point backbone](#gl-rbf-enhanced-point-backbone)
...
HDF5 datasets normally use stored trajectory partitions or, for a single unsplit trajectory, chronological 80/10/10 frame partitions. The explicit `legacy_seeded_random_frames` compatibility policy instead reproduces the historical turbulent-combustion PointCloudFFM split: eligible frame indices are shuffled by NumPy `default_rng(split_seed)`, the leading `int(N * train_ratio)` indices form training, the complement serves as both the historical validation and test aliases, and each selected subset is sorted before loading. This policy, seed, and ratio remain in the resolved child-run configuration so training previews and checkpoint evaluation select the same held-out frames.
